...
public class SitemapAction extends BaseAction {
	...
	public List<String> getListPage() throws Throwable {
		...
		try {
			IPage root = pageManager.getRoot();
			Lang lang = langManager.getDefaultLang();
			this.addPageUrl(root, urlList, null, urlManager, lang);
	...
	private void addPageUrl(IPage root, List<String> urlList, 
			IPage requestPage, IURLManager urlManager, Lang currentLang) {
		...
		for (int i=0; i<children.length; i++) {
			...
			this.addPageUrl(current, urlList, requestPage, urlManager, currentLang);
		}
	}
