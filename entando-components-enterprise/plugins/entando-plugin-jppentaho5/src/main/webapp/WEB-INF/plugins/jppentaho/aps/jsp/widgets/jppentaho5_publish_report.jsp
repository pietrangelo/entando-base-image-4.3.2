...
<%--
path: <c:out value="${pathVar}"/><br/>
args: <c:out value="${csvArgsVar}"/><br/>
cssClass: <c:out value="${cssClassVar}"/><br/>
--%>
...
<pth:report paramVar="parametersVar" urlVar="urlVar" path="${pathVar}" csvArgs="${csvArgsVar}">
			...
			<c:when test="${paramVar.attribute['parameter-render-type'].value == 'dropdown'}">
				...
				<c:forEach var="valueVar" items="${paramVar.values}">
					<!-- 
					 ...
					 -->
					  <c:set var="origName" value="${valueVar.value.label}" />
					 <c:set var="newValue" value="${fn:replace(origName, 
                                '&#x20;', ' ')}" />
					<option value="${valueVar.key}" <c:if test="${valueVar.value.selected}">selected="selected"</c:if> ><c:out value="${newValue}" /></option>
				</c:forEach>
				</select>
			...
			<c:when test="${paramVar.attribute['parameter-render-type'].value == 'list'}">
				...
				<c:forEach var="valueVar" items="${paramVar.values}">
					<!--
						value: <c:out value="${valueVar.key}" />, type: <c:out value="${valueVar.value.type}" />, selected: <c:out value="${valueVar.value.selected}" /><br/>
					-->
					  <c:set var="origNameL" value="${valueVar.value.label}" />
					 <c:set var="newValueL" value="${fn:replace(origNameL, 
                                '&#x20;', ' ')}" />
					<option value="${valueVar.key}" <c:if test="${valueVar.value.selected}">selected="selected"</c:if> ><c:out value="${newValueL}" /></option>
				</c:forEach>
				</select>
...
--%>
			<c:when test="${paramVar.attribute['parameter-render-type'].value == 'textbox'}">
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="valueVar" items="${paramVar.values}">
							<%-- FIXME select the first label even if is not selcted by default? --%>
							<c:if test="${valueVar.value.selected}"> <c:set var="defaultTextVar" value="${valueVar.value.label}"/> </c:if>
						</c:forEach>
					</c:otherwise>
				</c:choose>
			...
			<c:otherwise>
				<p>
					unknown render type: <c:out value="${paramVar.attribute['parameter-render-type'].value}"/>
...
<c:if test="${urlVar != 'none'}">
	...
	<c:when test="${not empty cssClassVar}">
		<iframe height="1000px" width="100%" src="${urlVar}&output-target=table/html;page-mode=stream" class="${cssClassVar}" frameborder=0 >
		</iframe>
	</c:when>
	<c:otherwise>
		<iframe height="1000px" width="100%" src="${urlVar}&output-target=table/html;page-mode=stream" class="jppentaho5" frameborder=0 >
		</iframe>
	</c:otherwise>
	</c:choose>
</c:if>
