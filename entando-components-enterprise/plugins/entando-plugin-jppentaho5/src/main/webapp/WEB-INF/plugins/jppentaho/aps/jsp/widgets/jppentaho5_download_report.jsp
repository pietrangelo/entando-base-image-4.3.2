...
<p>
	<wp:i18n key="jppentaho5_DOWNLOAD_REPORT" />
</p>
...
<pth:downloadReport paramVar="parametersVar" urlVar="urlVar" path="${pathVar}" csvArgs="${csvArgsVar}">
			...
			<c:when test="${paramVar.attribute['parameter-render-type'].value == 'list'}">
				...
				<c:forEach var="valueVar" items="${paramVar.values}">
					...
					<c:set var="origNameL" value="${valueVar.value.label}" />
					 <c:set var="newValueL" value="${fn:replace(origName, 
                                '&#x20;', ' ')}" />
...
--%>
	...
	<button value="submit"><wp:i18n key="jppentaho5_GENERATE_REPORT" /></button>
</form>
</pth:downloadReport>

<c:if test="${urlVar != 'none'}">
	
	<a href="${urlVar}" target="_blank"><wp:i18n key="jppentaho5_DOWNLOAD" /></a> 
	
	<%-- 
	<form method="link" action="${urlVar}">
