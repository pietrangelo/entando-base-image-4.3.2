...
<h1 class="panel panel-default title-page">
	<span class="panel-body display-block">
		<a href="<s:url action="list" namespace="/do/jpmultisite/Content"/>">
...
<div id="main" role="main">
	...
	<s:url namespace="/do/jpmultisite/Content/Ajax"  action="autosave" var="dataAutosaveActionVar" />
	<s:form cssClass="tab-container action-form" data-form-type="autosave" data-autosave-action="%{#dataAutosaveActionVar}">
		...
		<div id="info" class="panel panel-default"><%-- info section --%>
			...
			<div class="panel-body">
				<fieldset class="col-xs-12"><%-- share content with --%>
					<legend><s:text name="label.sharedContent" /></legend>

					<s:set var="multisiteListVar" value="multisiteList" scope="request" />	
					<s:if test="multisiteList.size > 0">
						<div class="form-group">
							<label for="extraGroups" class="basic-mint-label">
								<s:text name="label.join" />&#32;<s:text name="label.multisite" />
							</label>
							<div class="input-group">
								<select id="jpmultisite_multisites" name="shareWithMultisite" class="form-control">
									<c:forEach items="${multisiteListVar}" var="multisiteCodeVar" varStatus="status">
										...
										<option value="<c:out value="${multisiteCodeVar}"/>">
					...
					<%-- multisite added --%>
					<s:if test="content.sharedContent.size != 0">
						<div class="form-group">
							<div class="input-group">
								<s:iterator value="content.sharedContent" var="sharedContentVar">
									<wpsa:actionParam action="removeMultisite" var="actionName" >
										<wpsa:actionSubParam name="shareWithMultisiteRemove" value="%{#sharedContentVar.multisiteCodeTo}" />
									</wpsa:actionParam>
									<span class="label label-default label-sm pull-left padding-small-top padding-small-bottom margin-small-right margin-small-bottom">
										<span class="icon fa fa-tag"></span>&#32;
										<s:property value="%{getMultisiteTitle(#sharedContentVar.multisiteCodeTo)}"/>&#32;
										<wpsf:submit type="button" cssClass="btn btn-default btn-xs badge" action="%{#actionName}" title="%{getText('label.remove')+' '+#sharedContentVar.multisiteCodeTo}">
				...
				</fieldset><%-- share content with --%>
			</div>

			<div class="panel-body">
				<fieldset class="col-xs-12"><%-- view only groups --%>
				...
				</fieldset><%-- view only groups --%>
			</div>
			<%-- categories --%>
			<div class="panel-body">
				<s:action name="showCategoryBlockOnEntryContent" namespace="/do/jpmultisite/Content" executeResult="true">
