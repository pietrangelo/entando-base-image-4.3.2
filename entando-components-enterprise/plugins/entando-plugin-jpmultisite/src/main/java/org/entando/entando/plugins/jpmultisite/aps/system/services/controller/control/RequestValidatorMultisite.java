 ...
 */
public class RequestValidatorMultisite extends AbstractControlService {

	private static final Logger _logger = LoggerFactory.getLogger(RequestValidatorMultisite.class);
...
//					login service notfound
				} else if (!this.getErrorPageCode().equals(page.getCode()) && !this.getNotFoundPageCode().equals(page.getCode()) && !page.getCode().endsWith(MultisiteUtils.getMultisiteCodeSuffix(reqCtx.getRequest()))) {
					retStatus = this.redirect(this.getNotFoundPageCode(), reqCtx);
	...
	private boolean isRightPath(RequestContext reqCtx) {
		...
		if (this.getResourcePath(reqCtx).equals("/pages")) {
			...
			matcher = this._patternFullPathAt.matcher(resourcePath);
			...
			matcher = this._patternAt.matcher(resourcePath);
			if (matcher.lookingAt()) {
				ok = true;
				String sect1 = matcher.group(1);
				...
				matcher = this._oldPatternAt.matcher(resourcePath);
				if (matcher.lookingAt()) {
					ok = true;
					String sect1 = matcher.group(1);
					String sect2 = matcher.group(2);
	...
	@Deprecated
	protected Pattern _oldPatternAt = Pattern.compile("^/(\\w+)/(([a-zA-Z0-9_@])+)\\Q.wp\\E");
	
	protected Pattern _patternAt = Pattern.compile("^/(\\w+)/(([a-zA-Z0-9_@])+)\\Q.page\\E");
	
	protected Pattern _patternFullPathAt = Pattern.compile("^/pages/(\\w+)((/([a-zA-Z0-9_@])+)*)");
