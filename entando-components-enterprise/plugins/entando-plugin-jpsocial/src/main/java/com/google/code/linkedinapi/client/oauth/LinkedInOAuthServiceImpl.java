...
class LinkedInOAuthServiceImpl implements LinkedInOAuthService {
	 ...
	 */
	@Override
	public LinkedInAccessToken getOAuthAccessToken(LinkedInRequestToken requestToken, String scope, String oauthVerifier) {
	 ...
	 */
	@Override
	public LinkedInRequestToken getOAuthRequestToken(String callBackUrl, String scope) {
		try {
			final OAuthConsumer consumer = getOAuthConsumer();
			final OAuthProvider provider = getOAuthProvider(scope);
	...
	protected OAuthProvider getOAuthProvider(String scope) {
		DefaultOAuthProvider provider = new DefaultOAuthProvider(LinkedInApiUrls.LINKED_IN_OAUTH_REQUEST_TOKEN_URL + ((scope != null) ? "?scope=" + scope.replace(' ', '+') : ""),
