...
<div class="jpsocial jpsocial_fblogin">
	<h1><wp:i18n key="jpsocial_LOGIN_TITLE" />&nbsp;<wp:i18n key="jpsocial_LOGIN_FACEBOOK" /></h1>
	<c:choose>
		<c:when test="${authorizedUser}">
			<p>
				<wp:i18n key="WELCOME" />&#32;
				<em title="<c:out value="${sessionScope.currentUser.username}" /> / <c:out value="${sessionScope.currentUser.provider}" />"><c:out value="${sessionScope.currentUser}" /></em>!&#32;
				<a
					class="btn btn-small"
